...
- First-run onboarding clarity pass added on `2026-06-30`; the activation flow no longer has a bypass into the main app, the trip-source step requires a valid Google Maps viewing link plus manager name and age, and returning users with completed setup skip the wizard and enter the map/chat core. Local build, QA, and local browser smoke passed.
