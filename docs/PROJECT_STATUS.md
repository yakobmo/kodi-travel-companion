...
WhatsApp connector direction: WhatsApp is a future transport channel into the same Kodi/group context, not a separate bot brain. V1 dry connector added on 2026-07-07 with safe readiness and webhook endpoints, parser for Meta WhatsApp Cloud API text payloads, masked phone identifiers, and dry routing plans only. V2 live-capable bridge added on 2026-07-07: when Meta credentials are configured, inbound WhatsApp text is stored as a normal group message, Kodi is called through the same `/api/agent/message` pipeline, Kodi's reply is stored in the group chat, and the reply is sent back through Meta WhatsApp Cloud API. When not configured, the endpoint remains dry-run and does not write to chat. Public smoke passed after V2 deploy: `/api/health` returned OK, `/api/whatsapp/readiness` returned `disabled`/`dry_run` without secrets, and webhook POST parsed one sample text message while leaving the group chat unchanged.
WhatsApp inbound diagnostics added on 2026-07-07: `/api/whatsapp/diagnostics` reports recent webhook summaries with parsed text-message count, status-event count, message types, and sanitized text previews. This separates real inbound user messages from Meta delivery/status callbacks during setup without exposing tokens or full phone numbers.
WhatsApp webhook ACK correction added on 2026-07-07: live inbound webhooks now respond to Meta immediately after parsing and diagnostics, then process Kodi/store/outbound work in the background. This avoids Meta webhook timeouts when the AI path is slow or Render is waking.
WhatsApp webhook path hardening added on 2026-07-07: the API now accepts the canonical `/api/whatsapp/webhook` plus fallback aliases `/whatsapp/webhook`, `/api/webhook`, and `/webhook`, and records the hit path in diagnostics. This reduces setup fragility when Meta stores or the user enters a shortened callback URL during testing.
WhatsApp Meta Graph diagnostics added on 2026-07-07: `/api/whatsapp/meta-diagnostics` checks the configured WhatsApp Business Account subscribed apps and phone numbers from the server-side access token, returning masked environment identifiers only. This helps distinguish local webhook correctness from a missing/incorrect Meta WABA subscription.
WhatsApp readiness contract tightened on 2026-07-07: `/api/whatsapp/readiness` now distinguishes Render configuration from live Meta Graph readiness. A configured-but-expired Meta token reports `configured_but_not_live` with `expired_or_invalid_access_token` and `nextAction=replace_whatsapp_access_token_with_permanent_system_user_token` instead of presenting WhatsApp as fully ready.
WhatsApp readiness root-cause QA on 2026-07-07: a clean local API instance with an intentionally invalid Meta token returned `configured_but_not_live`, `expired_or_invalid_access_token`, and `phone_numbers_not_reachable`. This verifies the app now exposes the actual Meta blocker instead of mislabeling configuration as a working WhatsApp agent.
Public smoke after deploy on 2026-07-07: `/api/health` returned commit `2ca0e3d`, `/api/whatsapp/readiness` returned `configured_but_not_live`, and `/api/whatsapp/meta-diagnostics` showed Meta Graph `code 190` / `error_subcode 463` because the configured `WHATSAPP_ACCESS_TOKEN` session expired. Next WhatsApp step is replacing the temporary Meta token with a permanent System User token in Render, then running `npm run smoke:whatsapp`; do not redo webhook setup unless readiness later reports a webhook-specific blocker.
WhatsApp WABA subscription root fix prepared on 2026-07-08: live readiness now auto-ensures `/{WHATSAPP_BUSINESS_ACCOUNT_ID}/subscribed_apps` through Meta Graph and no longer reports `live_ready` merely because any subscribed app exists. The concrete failure found was a valid token and phone number with the WABA subscribed to a different Meta app (`WA DevX Webhook Events 1P App`) instead of Kodi, so readiness must require successful subscription of the WABA to the Kodi app before declaring WhatsApp live.
Public smoke after WhatsApp WABA subscription fix on 2026-07-08: Render served commit `01ba8a7`; `scripts/smoke-whatsapp-readiness.mjs --require-live` passed with `subscriptionEnsured=true`; `/api/whatsapp/meta-diagnostics` showed `subscriptionEnsure.status=200`, payload `{ success: true }`, and subscribed apps now include `Kodi AI Smart Guide` plus the previous Meta test app. The remaining live validation is sending a real WhatsApp message to the test number and confirming `/api/whatsapp/diagnostics` records the inbound POST after the Render instance is awake.

Kodi provider-failure correction added on 2026-07-08: WhatsApp diagnostics confirmed live transport was receiving and processing inbound messages, while agent provider readiness previously showed Gemini quota/provider failure being hidden behind a deterministic rules reply. `/api/agent/message` now exposes configured-provider failure as `source=agent_unavailable` with `agentRuntime.providerFailureVisible=true` instead of disguising it as a normal Kodi answer. Live agent-provider smoke is currently unstable: one run returned Gemini ready as `openAiModel=gemini:gemini-2.5-flash`, `source=openai`, and `fallbackUsed=false`, while follow-up runs returned Gemini HTTP 429 quota and `source=agent_unavailable`. This means WhatsApp transport is not the blocker; stable Kodi behavior now depends on fixing the configured AI provider quota/billing/key or replacing it with a provider key that has reliable quota.
Provider-quota stability correction added on 2026-07-13: Gemini fallback now stops model-candidate retries immediately after quota/billing errors, and `agent_unavailable` replies classify provider failures as quota/auth/not-configured/timeout/unknown with clear Hebrew admin instructions and Gemini/OpenAI links. This reduces quota burn and prevents the generic "Kodi is unavailable" answer from hiding what the admin must fix in Render or the AI provider account.

Google-powered Kodi architecture decision added on 2026-07-08: Kodi remains the travel agent, Google Maps Platform APIs are the official geographic source/tool layer, WhatsApp remains a protected communication channel into Kodi, and Gemini is only a future provider adapter option. The product must not depend on scraping or proxying the consumer Google Search / Google Maps AI answer box. See `docs/GOOGLE_POWERED_KODI_ARCHITECTURE.md`.
...
Public-list import correction completed on 2026-07-07: the saved Google Maps share link exposes a public `entitylist/getlist` response for `צפון יוון` with 79 declared items. Kodi now treats `imported_google_public_list` and `syncMode=google_public_list_read` as the required startup-sync evidence; falling back to the stale fixture must be reported as a fallback, not as Google sync. Public smoke passed after deploy: `/api/health` returned OK, `POST /api/trips/demo/google-source/sync` returned `imported_google_public_list`, `google_public_list_read`, and 79 places, and `/api/trips/demo/places` returned 79 places.
...
Live-location precision correction: Kodi must not answer location questions with raw coordinates when Google can translate the point. The web app now sends the fresh GPS reading with `accuracyMeters` and `updatedAt`; identity questions such as "where am I / which settlement / address / street" trigger a deterministic Google reverse-geocode / nearby-Places answer, while "near me" recommendations keep a practical Google Places radius so Kodi can find useful nearby bakeries, attractions, food, fuel, or services. If Google Geocoding is not enabled for the server key, Kodi should still answer from nearby Google Places without exposing raw latitude/longitude as the main user-facing answer.
Public smoke passed on 2026-07-07 after the live-location precision correction: `/api/health` returned OK; a public `/api/agent/message` current-location identity check returned a deterministic `group_location` answer with a human nearby Google place/address, GPS accuracy, timestamp, Google Maps link, and Waze link without raw coordinate leakage; Google public-list sync still returned `imported_google_public_list`, `google_public_list_read`, and 79 places.
...
- Participant WhatsApp welcome handoff added on 2026-07-09: the join screen now accepts an optional WhatsApp phone number. New members still receive the in-app Kodi welcome message, and the backend also attempts a personal WhatsApp welcome with the app link and a suggestion to save Kodi as a contact. Duplicate joins reuse the existing member and do not resend the welcome. The WhatsApp send is best-effort and depends on the configured Meta token/phone plus WhatsApp's conversation/template rules.
- Public smoke passed on 2026-07-09 after the participant WhatsApp welcome handoff: Render served commit `c47e770`, `/api/health` returned OK, the public web bundle included the join WhatsApp phone field, `/api/trips/demo/storage` confirmed relational Supabase storage, and `/api/trips/demo/members` safely rejected an invalid one-letter join request without polluting the group.
...
- Kodi default conversation routing corrected on 2026-07-06 and updated again on 2026-07-12: the web chat no longer limits Kodi to explicit wake-word messages. Normal trip/group messages are routed to the agent by default so Kodi can reason from context and respond naturally when useful.
...
- Kodi conversation routing correction updated on `2026-07-06` and aligned again on `2026-07-12`; Kodi is no longer limited to explicit wake-word messages. Normal trip/group messages route to the agent by default so the agent can reason from full group context.
...
- WhatsApp root-cause diagnostics added on `2026-07-08`: `/api/whatsapp/diagnostics` now reports live Meta readiness, WABA subscription ensure status, recent webhook summaries, and recent background processing results. `scripts/smoke-whatsapp-webhook-live.mjs` safely verifies the public webhook path with `dryRun=1` so smoke tests do not create fake chat members or messages.
