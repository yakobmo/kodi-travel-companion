...
- Kodi fast-answer lane added on 2026-07-01: simple lodging + nearby food/taverna questions now resolve from the trip timeline and guarded Google Places before invoking the full OpenAI agent, with `latencyMs` telemetry and a Places timeout so the chat stays responsive.
