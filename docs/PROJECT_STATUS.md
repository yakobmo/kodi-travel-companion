...
- Chat stability correction added on 2026-07-02: incoming SSE/polling message refreshes now merge into the local conversation instead of replacing pending messages, the mobile composer is sticky at the bottom of the contained chat surface, and direct presence pings such as "קודי?" return a server-side fast reply instead of drifting into stale route/trip context.
- Public smoke passed on 2026-07-02 after the chat stability correction: `/api/agent/message` returns `source=fast_presence` for "קודי?", and the public CSS bundle contains the sticky composer / fixed root overflow rules.
- Kodi follow-up wake correction added on 2026-07-06: after a pure presence ping such as "קודי?", the next user message is treated as the real Kodi request even if it does not repeat the wake word, while direct family chat such as "מה קורה אורייה" still stays in the group. Public smoke confirmed `/api/health`, `/api/trips/demo/storage`, and `/api/agent/message` with `source=fast_presence` and the new follow-up text on Render.
- Kodi presence-gate root fix added on 2026-07-06: actionable requests such as "קודי אתה יכול לעשות לי סדר במקומות לינה?" no longer get misclassified as a fast presence ping just because they contain "אתה". Public smoke confirmed the same request now reaches OpenAI (`source=openai`, `openAiStatus=ready`) while "קודי?" still returns the fast presence response.
...
- Kodi wake gate correction updated on `2026-07-05`; Kodi now wakes only when a participant explicitly addresses "קודי" / "קודקס" / Kodi / Codex, while ordinary group messages stay inside the family chat even if Kodi replied moments earlier. Voice conversation mode may still force a Kodi reply because the user explicitly opened that mode. Regression smoke now verifies that a message such as "מה קורה אורייה" does not summon Kodi.
- Hebrew Kodi agent master specification added on `2026-07-05` in `docs/KODI_AGENT_MASTER_SPEC_HE.md`; it consolidates the agent identity, Google Maps-first knowledge model, wake-word rule, here-and-now behavior, trip timeline intelligence, pragmatic Google Maps/Waze actions, voice expectations, failure modes, and mandatory agent QA scenarios. Future Kodi prompt/code changes should be checked against this document before implementation.
