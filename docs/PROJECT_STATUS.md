...
WhatsApp readiness contract tightened on 2026-07-07: `/api/whatsapp/readiness` now distinguishes Render configuration from live Meta Graph readiness. A configured-but-expired Meta token reports `configured_but_not_live` with `expired_or_invalid_access_token` instead of presenting WhatsApp as fully ready.
...
Public smoke after deploy on 2026-07-07: `/api/health` returned commit `2ca0e3d`, `/api/whatsapp/readiness` returned `configured_but_not_live`, and `/api/whatsapp/meta-diagnostics` showed Meta Graph `code 190` / `error_subcode 463` because the configured `WHATSAPP_ACCESS_TOKEN` session expired. Next WhatsApp step is replacing the temporary Meta token with a production/permanent token path, not more webhook reconfiguration.
