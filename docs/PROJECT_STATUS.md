...
- Google Maps runtime config correction added on `2026-06-30`; local API build, web build, QA, local smoke, and diff check passed. Public smoke must verify `/api/config/maps` after Render deploy and should show `configured=true` only after a browser-safe Google Maps key is configured in Render.
